...
# ---------- 시트 선택 버튼 UI (정렬 개선: 균등 너비 그리드) ----------
st.subheader("시트 선택")
PER_ROW = 6  # 한 줄에 6개
for i in range(0, len(sheet_names), PER_ROW):
    row_names = sheet_names[i:i+PER_ROW]
    cols = st.columns(PER_ROW, gap="small")
    for j, c in enumerate(cols):
        if j < len(row_names):
            name = row_names[j]
            if c.button(name, key=f"sheetbtn-{i+j}", use_container_width=True):
...
# 지니/로렌츠 (상속재산 & 결정세액) — 색상 고정, x/y 라벨 제거
...
with col1:
    ...
    if shares is not None:
        ...
        # plt.xlabel("")
        # plt.ylabel("")
        # plt.title removed to avoid garbled text
        plt.tight_layout()
        st.pyplot(fig)
...
st.markdown("> **설명:** 파랑=상속재산 기준, 빨강=결정세액 기준. 값은 소수 셋째자리로 표시됩니다.")
...
st.markdown(
    ...
    **지니계수(Gini coefficient)**는 분배의 불평등 정도를 0에서 1 사이의 값으로 나타내는 지표입니다.  
    ...
    - **본 앱의 계산 방식:** 각 시트의 **분위(상위 10%~100%)**별 **점유비(%)** 또는 **금액 비중**을 바탕으로  
      로렌츠 곡선을 구성하고, 수치적분(면적)을 이용해 지니계를 산출합니다.
    - **해석 팁:**  
